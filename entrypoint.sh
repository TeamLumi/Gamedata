...
then
  ...
  REMOTE_COMMIT=$(git ls-remote origin $OUTPUT_BRANCH | awk '{print $1}')

  # Check if the remote commit matches the local commit
  if [ "$LOCAL_COMMIT" = "$REMOTE_COMMIT" ]; then
    echo "Commit successfully pushed to remote repository."
  else
    echo "Commit not found on remote. Retrying..."
